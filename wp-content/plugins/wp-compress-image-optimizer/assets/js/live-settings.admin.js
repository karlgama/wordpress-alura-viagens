jQuery(document).ready(function ($) {
    ...
    function ExcludeList() {
        ...
        $(save).on('click', function (e) {
            ...
            $.post(wps_ic_vars.ajaxurl, {action: 'wps_ic_exclude_list', excludeList: excludeList, lazyExcludeList:lazyExcludeList}, function (response) {
                if (response.success) {
     ...
     */
    $('.wpc-default-settings').on('click', function (e) {
        e.preventDefault();
        ...
        var settings_area = $(this).data('settings-area');


        $.post(wps_ic_vars.ajaxurl, {action: 'wps_ic_get_default_settings', area: settings_area}, function (response) {

            if (response.success) {
                var settings = response.data;

                if (settings_area == 'serve-cdn') {
                    $('input[type="checkbox"]', '.settings-area-serve-cdn').each(function (i, item) {
                        var checkbox_status = $(item).is('checked');
                        var setting_name = $(item).data('setting_name');
                        var setting_current_status = $(item).is(':checked');
                        var default_status = settings[setting_name];

                        if (default_status != setting_current_status) {
                            if (default_status == '1') {
                                // ON
                    ...
                    $('input[type="checkbox"]', '.settings-area-additional-settings').each(function (i, item) {
                        var checkbox_status = $(item).is('checked');
                        var setting_name = $(item).data('setting_name');
                        var setting_current_status = $(item).is(':checked');
                        var default_status = settings[setting_name];
    ...
    */
    $('#circle-big').circleProgress({
        size: 160, startAngle: -Math.PI / 6 * 3, lineCap: 'round', thickness: '6', fill: {
