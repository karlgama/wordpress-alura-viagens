...
class wps_ic_mu extends wps_ic {
	...
	public function __construct() {
		$this->default_settings = array('live-cdn'          => '0',
																		'js'                => '0',
																		'css'               => '0',
																		'css_image_urls'    => '0',
																		'external-url'      => '0',
		...
		add_action('wp_initialize_site', array($this, 'new_mu_site'), 900);
		...
		if ( ! empty($_POST['cname'])) {
			...
			if (empty($error)) {
				...
				if ( ! preg_match('/^([a-zA-z0-9\_\-]+)\.([a-zA-z0-9\_\-]+)\.([a-zA-z0-9\_\-]+)$/', $cname, $matches) && ! preg_match('/^([a-zA-z0-9\_\-]+)\.([a-zA-z0-9\_\-]+)\.([a-zA-z0-9\_\-]+)\.([a-zA-z0-9\_\-]+)$/', $cname, $matches)) {
					...
					$call  = wp_remote_get($verify_cname_dns, array('timeout' => 60, 'sslverify' => false, 'user-agent' => $agent));
					if (wp_remote_retrieve_response_code($call) == 200) {
	...
	public function mu_save_default_settings() {
		...
		foreach ($saved_settings as $key => $value) {
			 ...
			 */
			if ( ! empty($saved_settings[ $key ]) && empty($form_settings[ $key ])) {
				$form_settings[ $key ] = 0;
			}
		}
	...
	public function get_agency_stats() {
		...
		$get = wp_remote_get('https://app.wpcompress.com/?token=' . $token . '&action=details', array('timeout' => 10));
	}
	...
	public function mu_disconnect_single_site() {
		...
		wp_send_json_success(array('html_status' => '<a href="#" class="wps-ic-mu-connect wpc-mu-individual-connect-bulk hvr-grow" data-site-id="' . $siteID . '"><i class="icon icon-link"></i> Connect</a>'));
		...
		if (wp_remote_retrieve_response_code($get) == 200) {
			...
			if ($body->success == true && $body->data->apikey != '' && $body->data->response_key != '') {
				...
				if ($single) {
					...
					wp_send_json_success(array('html_status' => '<a href="#" class="wps-ic-mu-configure ic-tooltip" title="Configure" data-site-id="' . $siteID . '"><i class="icon icon-cog"></i></a><a href="#" class="wps-ic-mu-disconnect wpc-mu-individual-disconnect-bulk ic-tooltip" title="Disconnect" data-site-id="' . $siteID . '"><i class="icon icon-cancel"></i></a>'));
				}
				...
				$reconnect_msg = 'api-error';
				wp_send_json_error(array('msg' => $reconnect_msg, 'body' => $body_msg));
			}
		}
		else {
			$reconnect_msg = 'api-error';
	...
	public function mu_connect() {
		...
		$get = wp_remote_get('https://app.wpcompress.com/?token=' . $token . '&site_url=' . $site_url, array('timeout' => 10));
